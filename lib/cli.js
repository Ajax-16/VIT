 ...
 *   node index.js promote --bump minor --yes
 *   node index.js sync
...
//   prereqBranch — if true, only shown in the interactive menu when on a
//                  prerelease branch (detected at runtime in index.js)
 ...
 *   promote  — needs: command + bump + yes
 *   sync     — always headless (no prompts needed)
 */
function resolveHeadless(ctx) {
  if (!ctx.command) return false;
  if (ctx.command === "sync") return true;
  ...
  switch (ctx.command) {
    ...
    case "promote":  return Boolean(ctx.bump);
...
export function printHelp() {
  ...
  lines.push(
    ...
    "    vit promote --bump minor --yes",
    "    vit promote --bump patch --target main --yes",
    "    vit sync",
    "    vit sync --dry-run",
    "",
  );
