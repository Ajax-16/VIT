...
/**
 * If config.changelog.path is set, resolves it directly without prompting.
 * Only asks the user when the path is missing from the config.
 */
async function resolveChangelogPath(config) {
  if (config.changelog?.path) {
    return resolve(config.changelog.path);
  }
...
export async function buildChangelog(config) {
  ...
  const CHANGELOG_PATH = await resolveChangelogPath(config);
...
export async function buildSemanticChangelog(config, opts = {}) {
  ...
  console.log(
    chalk.dim(`  Found ${tagEntries.length} tag(s) · ${totalCommits} recognised commit(s) total\n`)
  );
