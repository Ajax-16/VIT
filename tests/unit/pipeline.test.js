...
function makeChild({ stdout = '', stderr = '', exitCode = 0, signal = null } = {}) {
...
describe('runSteps', () => {
  ...
  test('interpolates ${VAR} in command before spawning', async () => {
    ...
    await runSteps(
      [{ command: 'echo ${GREETING}', captureAs: 'OUT' }],
  ...
  test('rejects on non-zero exit when continueOnError is false', async () => {
    mockSpawn.mockImplementationOnce(() => makeChild({ exitCode: 1, stderr: 'something failed' }));
    await expect(
