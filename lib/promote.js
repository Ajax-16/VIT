...
function isDirty(cwd = process.cwd()) {
  try {
    return git("git status --porcelain", cwd).length > 0;
  } catch {
    return false;
  }
}
...
function headSha(branch, cwd = process.cwd()) {
  return git(`git rev-parse ${branch}`, cwd);
}

/**
 * Hard-resets a branch to a given SHA and force-pushes it.
 * Used for rollback after a failed promote.
 */
function rollback({ preBranch, preSha, preTargetSha, branch, targetBranch, vcs }) {
  try {
    vcs.checkout(targetBranch);
    git(`git reset --hard ${preTargetSha}`);
    git(`git push --force-with-lease origin ${targetBranch}`);
  } catch { /* best-effort */ }
  try {
    vcs.checkout(branch);
    git(`git reset --hard ${preSha}`);
    git(`git push --force-with-lease origin ${branch}`);
  } catch { /* best-effort */ }
...
function detectGithubRepo(cwd = process.cwd()) {
  try {
    ...
    const match = remote.match(/github\.com[:/]([^/]+\/[^/]+?)(\..+)?$/) ?? null;
 ...
 *   - Snapshots HEAD SHAs of both branches for rollback.
 *
 * Steps:
 *   1. Checkout target branch.
 *   2. Merge prerelease branch into target (--no-ff for traceability).
 *   3. Bump stable version on target.
 *   4. Push target branch with tags.
 *   5. Sync prerelease branch: ff-only first, merge fallback.
 *   6. Push prerelease branch.
 *
...
export async function promoteMerge({
  ...
  // ─ Pre-flight ───────────────────────────────────────────────────────────────────
  if (isDirty()) {
    spinner.fail(chalk.red("Working tree has uncommitted changes. Please commit or stash before promoting."));
    throw new Error("Dirty working tree");
  }

  const preBranch = git("git rev-parse --abbrev-ref HEAD");
  const preSha    = headSha(branch);
  const preTargetSha = headSha(targetBranch);
  ...
  try {
    vcs.checkout(targetBranch);
    execSync(`git merge --no-ff ${branch} -m "chore: merge ${branch} into ${targetBranch} for promotion"`, { stdio: "pipe" });
  ...
  try {
    git(`git merge --ff-only ${targetBranch}`);
    syncSpinner.succeed(chalk.green(`${branch} synced to ${targetBranch} (fast-forward).`));
  } catch {
    try {
      git(`git merge ${targetBranch} -m "chore: sync ${branch} with ${targetBranch}"`);
