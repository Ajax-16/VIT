...
export const VALID_TRIGGERS = ["release", "prerelease", "commit", "changelog"];
...
export function interpolateCommand(command, env) {
  ...
  return command.replace(/\$\{([^}]+)\}/g, (_, key) => {
    const value = env[key] ?? "";
    // Escapar backslashes para que el shell no los interprete como escapes
...
export async function buildActionRuntime(
  ...
  const builtins = resolveVitBuiltins(
    action.cwd,
    configProjects,
    getVcsAdapter(configVcs),
  );
