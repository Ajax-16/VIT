...
// En changelog.js — helper para detectar si un tag es pre-release
function isPreReleaseTag(tagName, preReleaseBranches = []) {
  // Extraer la parte de versión quitando cualquier prefijo (v-, core-, etc.)
  const versionPart = tagName.replace(/^[a-zA-Z]+-?/, "");

  // Si semver lo parsea directamente
  if (semver.prerelease(versionPart)) return true;

  // Fallback: comprobar si contiene algún id de pre-release conocido
...
export function parseAllTagsWithCommits(
  ...
  if (pendingTag) {
    const mostRecentTag =
      tags.find((t) => !isPreReleaseTag(t, config.git?.preReleaseBranches)) ??
      null;
...
export async function buildSemanticChangelogInteractive(config, pendingTag) {
  ...
  if (candidates.length === 0) {
    console.log(
      chalk.yellow(
        "  ⚠ No conventional commits found since last tag. Skipping changelog.\n",
      ),
    );
 ...
 * | true     | false    | undefined  | ⚠ warn + return { saved: false }       |
 * | true     | false    | set        | buildSemanticChangelogInteractive      |
 * | true     | true     | any        | buildSemanticChangelogHeadless         |
 *
 * NOTE: interactive semantic mode (MODE 3) requires a `pendingTag` because it
 * regenerates the full changelog including the version about to be released.
 * Without it there is nothing meaningful to do — the function returns
 * { saved: false } immediately without opening any prompts.
 *
 * @param {object}  config
 * @param {object}  opts
...
export async function runChangelog(config, opts = {}) {
  ...
  // MODE 3 — interactive semantic requires a pendingTag.
  // Without it (e.g. called as standalone `vit changelog`) we cannot build
  // a meaningful semantic entry — return { saved: false } without any prompts.
  if (!pendingTag) {
