...
export function parseAllTagsWithCommits(
  ...
  if (pendingTag) {
    const isPreReleaseTag = (t) => semver.prerelease(t) !== null;
    const lastStableTag = tags.find((t) => !isPreReleaseTag(t)) ?? null;
    const mostRecentTag = lastStableTag;
...
export function parseCommitsSinceLastTag(config, cwd = process.cwd()) {
  const knownTypes = new Set(config.types.map((t) => t.value));
  const tags = getAllTagsSorted(cwd);
  const lastTag = tags.find((t) => !semver.prerelease(t)) ?? null;
