  ...
  return actions
    ...
    .filter((action) => {
      ...
      if (!action.command || typeof action.command !== "string" || action.command.trim() === "") {
        console.warn(
          chalk.yellow(`  ⚠ Action "${action.label}" (id: ${action.id}) has no command and will be skipped.`)
        );
        return false;
      }
 ...
 */
export async function buildActionRuntime(action, pipelineLabel = "pipeline", globalEnvFile = null) {
  // 1. Prompt first — always before any spawn
  const promptedEnv = await resolvePromptEnv(action);

  // 2. Load global envFile (lowest extra priority)
  const globalFileEnv = globalEnvFile ? loadEnvFile(globalEnvFile) : {};

  // 3. Load action-level envFile (overrides global envFile)
  const actionFileEnv = action.envFile ? loadEnvFile(action.envFile) : {};

  // 4. Merge in priority order:
  //    process.env < globalEnvFile < action.envFile < action.env < promptedEnv
  const mergedEnv = {
    ...process.env,
    ...globalFileEnv,
    ...actionFileEnv,
  ...
  // 5. Interpolate all string values
  ...
  // 6. Run pipeline steps — pass action.cwd so steps inherit it when they have no explicit cwd
  const pipelineEnv = await runSteps(action.pipeline, interpolatedEnv, pipelineLabel, action.cwd);

  // 7. Interpolate command with pipeline-enriched env
...
export async function runActions(actions, trigger, sectionLabel = "actions", globalEnvFile = null) {
  const applicable = getApplicableActions(actions, trigger);
  if (applicable.length === 0) return;
  ...
  for (const action of applicable) {
    try {
      const runtimeAction = await buildActionRuntime(action, `${sectionLabel} pipeline`, globalEnvFile);
