...
/**
 * bumpTypes that correspond to a prerelease cycle.
 * Used to resolve the actions trigger: "prerelease" vs "release".
 */
const PRE_BUMP_TYPES = new Set(Object.keys(PRE_BUMP_MAP));

/**
 * Returns the actions trigger that matches the given bumpType.
 *
 *   prepatch | preminor | premajor | prerelease  →  "prerelease"
 *   patch | minor | major | promote              →  "release"
 */
export function resolveActionsTrigger(bumpType) {
  return PRE_BUMP_TYPES.has(bumpType) ? "prerelease" : "release";
}
