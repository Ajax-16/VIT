 ...
 * 3. Open a GitHub PR (head = prerelease branch, base = target branch).
 * 4. Print the PR URL — human approves & merges from GitHub.
 *
 * Requires a GitHub token via GITHUB_TOKEN env var or config.github.token.
 */
export async function promotePr({
  ...
  const token = process.env.GITHUB_TOKEN ?? config.github?.token ?? null;
  ...
  const prSpinner = ora({ text: "Opening pull request...", color: "magenta" }).start();
  ...
  spinner.succeed(chalk.green("Promotion PR ready!"));
