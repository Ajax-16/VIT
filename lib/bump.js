...
export function getNextVersion(currentVersion, bumpType, preId = null) {
  if (bumpType === "prerelease") {
    ...
    // Si viene de estable, queremos 1.2.1-alpha.1 no 1.2.1-alpha.0
    if (parsed.prerelease.length === 0) {
      const next = semver.inc(currentVersion, "prerelease", preId ?? "pre");
      // next = "1.2.1-alpha.0" → lo convertimos a "1.2.1-alpha.1"
      const nextParsed = semver.parse(next);
      return `${nextParsed.major}.${nextParsed.minor}.${nextParsed.patch}-${preId ?? "pre"}.1`;
    }
