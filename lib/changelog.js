...
import { getVcsAdapter } from "./vcs/index.js";
...
export function parseAllTagsWithCommits(
  ...
  const adapter = getVcsAdapter(config.vcs?.provider);
  ...
  if (pendingTag) {
    ...
    const rawLines = adapter.getCommitsToHead(mostRecentStable);
  ...
  while (i < tags.length) {
    ...
    const date = adapter.getTagDate(tag);
    const rawLines = adapter.getCommitsBetween(prevStableTag, tag);
    const commits = rawLines
      .map(parseCommit)
      .filter((c) => c !== null && knownTypes.has(c.type));
...
export function parseCommitsSinceLastTag(config, cwd = process.cwd()) {
  ...
  const lastStableTag =
    tags.find((t) => !isPreReleaseTag(t, preReleaseBranches)) ?? null;
  const rawLines = adapter.getCommitsToHead(lastStableTag);
  return rawLines
    .map(parseCommit)
    .filter((c) => c !== null && knownTypes.has(c.type));
}
...
function getLatestTagEntry(config, cwd = process.cwd(), opts = {}) {
  ...
  const date = adapter.getTagDate(latestStable);
  const rawLines = adapter.getCommitsBetween(prevStableTag, latestStable);
  const commits = rawLines
    .map(parseCommit)
    .filter((c) => c !== null && knownTypes.has(c.type));
