...
function normalizeStep(step, index, parentCwd = null) {
  return {
    id: step.id ?? `step-${index + 1}`,
    label: step.label ?? step.command ?? `Step ${index + 1}`,
    command: step.command ?? "",
    captureAs: step.captureAs ?? null,
    // If step has no explicit cwd, inherit from parent action; fallback to process.cwd()
    cwd: step.cwd ?? parentCwd ?? process.cwd(),
...
/**
 * Runs an array of pipeline steps.
 ...
 * @param {string}   sectionLabel - Label shown in header
 * @param {string|null} parentCwd - cwd inherited from the parent action (if step has no explicit cwd)
 * @returns {object} Enriched env with all captureAs values merged in
 */
export async function runSteps(steps, baseEnv, sectionLabel = "pipeline", parentCwd = null) {
  const normalized = steps.map((s, i) => normalizeStep(s, i, parentCwd));
...
export function printStepsSummary(steps, indent = "      ", parentCwd = null) {
  if (!Array.isArray(steps) || steps.length === 0) return;
  steps.forEach((s, i) => {
    const step = normalizeStep(s, i, parentCwd);
