...
const DEFAULT_CONFIG = {
  ...
  git: {
    ...
    strict: false,
    // Rollback strategy:
    //   "revert" (default) — creates a new revert commit, history is preserved,
    //                        safe for shared repos, no force push needed.
    //   "reset"            — hard-resets HEAD to the tag, rewrites history,
    //                        requires force push. Use only on personal/solo repos.
...
export function loadVitConfig() {
  ...
  try {
    ...
    // Validate rollbackStrategy — only "revert" and "reset" are valid.
      ...
      git: {
        ...
        preReleaseBranches: Array.isArray(parsedGit.preReleaseBranches)
          ? parsedGit.preReleaseBranches
              ...
              .map((b) => ({
                name: b.name,
                id: typeof b.id === "string" ? b.id : b.name, // si no hay id, usa el nombre de la rama
              }))
