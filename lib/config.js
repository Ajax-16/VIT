...
const DEFAULT_CONFIG = {
  ...
  git: {
    ...
    // Promote strategy:
    //   "merge" (default) — merges prerelease branch into target locally,
    //                       bumps stable version, pushes, syncs both branches.
    //   "pr"              — bumps stable version on the prerelease branch,
    //                       then opens a GitHub PR via the GitHub API and
    //                       returns the PR URL. Requires GITHUB_TOKEN env var
    //                       or config.github.token, and config.github.repo
    //                       (or a valid git remote origin pointing to GitHub).
    promoteStrategy: "merge",
  },
  // GitHub API settings — required when git.promoteStrategy is "pr"
  github: {
    // Personal access token. Use the GITHUB_TOKEN environment variable
    // instead of hardcoding a value here — env takes precedence.
    token: null,
    // "owner/repo" slug. Auto-detected from git remote origin when null.
    repo: null,
  },
  vcs: {
    provider: "git",
