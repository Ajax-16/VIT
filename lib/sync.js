 ...
 *   - Single `fetch --tags --prune` keeps all remote refs + tags up to date.
 *   - Pulls each release branch before comparing so local refs are up to date.
 *   - commitsBehind uses origin/<branch> ref when available (no checkout needed).
 ...
 * Prefers origin/<branch> (updated by fetchAll) so no checkout is needed.
 */
function commitsBehind(branch, base) {
  try {
    const branchRef = (() => {
      try { git(`git rev-parse --verify origin/${branch}`); return `origin/${branch}`; }
      catch { return branch; }
    })();
    const count = git(`git rev-list --count ${branchRef}..${base}`);
...
/**
 * Single fetch that brings branches, tags and prunes dead remote refs.
 * Non-fatal: if offline or no remote, we continue with local state.
 */
function fetchAll() {
  try {
    git("git fetch --tags --prune origin");
  } catch { /* non-fatal — might be offline or no remote */ }
}
...
export async function runSync({ config, vcs, dryRun }) {
  ...
  for (const preBranch of preReleaseBranches) {
    ...
    // commitsBehind uses origin/<preBranch> — no checkout needed here
    let maxBehind = 0;
    ...
    const preSha = headSha(`origin/${preBranch}`) || headSha(preBranch);
    ...
    try {
      git(`git merge --ff-only origin/${preBranch}`);
    } catch { /* non-fatal — no remote ref or already up to date */ }
