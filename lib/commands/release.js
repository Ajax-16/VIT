...
export async function run({
  ...
  if (branch) {
    if (isOnPreReleaseBranch) {
      if (
        cli.bump &&
        !["prepatch", "preminor", "premajor", "prerelease"].includes(cli.bump)
      ) {
        console.log(
          "\n" +
      ...
      const { allowed } = checkReleaseBranch(
        config.git.releaseBranches,
        branch,
      );
      if (!allowed) {
        const allowed_list = config.git.releaseBranches
          .concat(config.git.preReleaseBranches.map((pre) => pre?.name ?? pre))
          .join(", ");
        if (config.git.strict && !dryRun) {
          ...
          console.log(
            "\n" +
              ...
              (isDryRunBypass
                ? chalk.dim(" (strict bypassed in dry-run)")
                : "") +
              "\n" +
  ...
  if (isOnPreReleaseBranch) {
    ...
    if (cli.bump) {
      ...
      if (!cli.yes)
        console.log(
          chalk.dim(`  Bump pre-selected: ${chalk.cyan(bumpType)}\n`),
        );
  ...
  if (!isPreIteration) {
    changelogDone = await runChangelogStep({
      cli,
      config,
      dryRun,
      accion: "release",
