...
/**
 * Parses conventional commits since the last git tag (or all commits if no
 * tag exists). Returns entries compatible with buildSection().
 *
 * Supported format: <type>[(<scope>)][!]: <description>
 *   feat(auth): add OAuth login
 *   fix!: correct null pointer
 */
export function parseCommitsSinceLastTag(cwd = process.cwd()) {
  let range = "HEAD";
...
/**
 * Semantic changelog: entries are derived automatically from conventional
 * commits since the last git tag.
 *
 * Behaviour depends on the `headless` option:
 *
 *   headless = false (interactive)
 *     — Shows a checkbox so the user can deselect commits.
 *     — Asks for an optional intro text.
 *     — Shows a preview and asks for confirmation before saving.
 *
 *   headless = true (CI / --yes)
 *     — Includes ALL recognised commits with no prompts.
 *     — Saves directly. No preview, no confirm.
 *
 * The `version` option can be supplied by the caller (headless mode) so the
 * function never needs to prompt for it in that case.
 *
 * @param {object} config
 * @param {{ headless?: boolean, version?: string }} [opts]
 * @returns {Promise<{ saved: boolean, path: string | null }>}
 */
export async function buildSemanticChangelog(config, opts = {}) {
  const { headless = false, version: versionOverride = null } = opts;

  const CHANGELOG_PATH = headless
    ? resolve(config.changelog.path)
    : await askPath(config);
  ...
  if (!version) {
    if (headless) {
      // In headless mode without a version, try to read it from package.json
      try {
  ...
  const skippedCount = allCommits.length - recognised.length;

  console.log(
    chalk.dim(
      `  Found ${allCommits.length} commit(s)` +
      `, ${recognised.length} recognised` +
      (skippedCount > 0 ? `, ${skippedCount} skipped (unknown type)` : "") +
      "\n"
    )
  );
  ...
  // ── Interactive: let user review / deselect commits ─────────────────
  let selected = recognised;
  let intro = null;
    ...
    const { selectedRaws } = await inquirer.prompt([
      {
        type: "checkbox",
        name: "selectedRaws",
        message: "Select commits to include:",
        choices: recognised.map((c) => ({ name: c.raw, value: c.raw, checked: true })),
      },
    ]);

    selected = recognised.filter((c) => selectedRaws.includes(c.raw));
  ...
  // ── Build section ─────────────────────────────────────────────
  const section = buildSection(version, selected, config, intro);
 ...
 * Available in both manual and semantic mode.
 */
