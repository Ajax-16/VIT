 ...
 *
 * Shape: Array<{ tag: string, date: string, commits: ParsedCommit[] }>
 *
 * Commits that don't follow the conventional format are silently discarded.
 * Tags whose commits are all unknown types (per config) are still included
 * but will produce an empty section.
 */
export function parseAllTagsWithCommits(config, cwd = process.cwd()) {
  const knownTypes = new Set(config.types.map((t) => t.value));
  const tags       = getAllTagsSorted(cwd); // most-recent first

  if (tags.length === 0) return [];

  // Build pairs: [newerTag, olderTag] so we can get the range in between.
  // The oldest tag uses null as `from` (= all commits up to it).
  const result = [];

  for (let i = 0; i < tags.length; i++) {
    const tag      = tags[i];
    const prevTag  = tags[i + 1] ?? null; // the tag just before this one
  ...
  // Commits after the latest tag (unreleased) are intentionally excluded
  // from the full regeneration — they belong to the next release.
...
function injectEntriesToSection(sectionRaw, newEntries, config, intro = null) {
  ...
  for (const [type, items] of sorted) {
    ...
    if (sectionRaw.includes(typeHeader)) {
      ...
      sectionRaw = sectionRaw.replace(blockRegex, (m) => m.trimEnd() + "\n\n" + newLines);
    } else {
      addition += `${typeHeader}\n\n${newLines}\n\n`;
    }
  }
  ...
  if (addition) {
    ...
    const reordered = TYPE_ORDER
      .filter((t) => existingGroups[t])
...
export async function buildSemanticChangelog(config, opts = {}) {
  ...
  // ── Parse all tags ─────────────────────────────────────────────────────────
  const tagEntries = parseAllTagsWithCommits(config);

  if (tagEntries.length === 0) {
    console.log(chalk.yellow("  ⚠ No tags found in the repository. Nothing to generate.\n"));
  ...
  console.log(
    chalk.dim(
      `  Found ${tagEntries.length} tag(s) · ${totalCommits} recognised commit(s) total\n`
    )
  );
  ...
  for (const { tag, date, commits } of tagEntries) {
    if (commits.length === 0) {
      // Include the tag header with a note so no release is silently missing.
      fullContent += `## [${tag}] - ${date ? formatDate(date) : "?"}\n\n`;
      fullContent += `> *No conventional commits found for this release.*\n\n\n`;
      continue;
    }
  ...
  if (!headless) {
    ...
    console.log(chalk.dim(previewLines.join("\n")));
