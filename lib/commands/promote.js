...
export async function run({ cli, config, vcs, dryRun, branch, isOnPreReleaseBranch, preId, semanticChangelog, promoteStrategy }) {
  ...
  // ── Dirty-tree check (must happen BEFORE changelog modifies any file) ─────────────────────
  if (!dryRun && vcs.isDirty()) {
    console.log(
      "\n" + chalk.bgRed.white.bold("  BLOCKED  ") + "  " +
      chalk.red.bold("Working tree has uncommitted changes. Please commit or stash before promoting.") + "\n",
    );
    process.exit(1);
  }
  ...
  // ── Changelog (working tree is clean at this point) ───────────────────────────────────
