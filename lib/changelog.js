 ...
 * follows them chronologically. This means the changelog only ever shows
 * stable releases, with all work done across prerelease iterations grouped
 * under the corresponding stable version.
 *
...
export function parseAllTagsWithCommits(
  ...
  // Pending entry: commits from last stable tag to HEAD
  if (pendingTag) {
  ...
  // Walk tags newest-first.
  // For each STABLE tag, collect commits from:
  //   - the previous STABLE tag (or the oldest prerelease in this cycle)
  //     all the way up to this stable tag.
  // Prerelease tags are skipped as standalone entries.
  let i = 0;
  while (i < tags.length) {
    const tag = tags[i];

    if (isPreReleaseTag(tag, preReleaseBranches)) {
      // Skip prerelease tags — their commits are covered by the stable tag
      // that came before them (remember the array is newest-first, so the
      // stable tag is at a lower index and was already processed, or this
      // prerelease is the leading edge and will be covered by pendingTag).
      i++;
      continue;
    }

    // This is a stable tag. Find the boundary: the previous stable tag.
    ...
    // Collect ALL commits between the previous stable tag and this stable tag,
    // which naturally includes any prerelease tags in between.
...
export function buildChangelogHeadless() {
  console.log(
    chalk.dim("  ℹ Changelog skipped (non-semantic headless mode).\n"),
  );
}
 ...
 *
 * Steps:
 *   1. Detect conventional commits between last stable tag and HEAD.
 *   2. Show checkbox list — user deselects commits to exclude.
 *   3. Ask for optional intro text.
 *   4. Regenerate full changelog (stable tags only + new pending section).
...
export async function buildSemanticChangelogInteractive(config, pendingTag) {
  ...
  const tagEntries = parseAllTagsWithCommits(config, process.cwd(), {
    pendingTag,
  });
 ...
 * Regenerates the full changelog from all STABLE tags only.
 * Prerelease tags are skipped (their commits accumulate in the stable entry).
 */
export async function buildSemanticChangelogHeadless(config, opts = {}) {
  ...
  writeFileSync(CHANGELOG_PATH, fullContent, "utf-8");
  spinner.succeed(chalk.green(`Changelog regenerated at: ${CHANGELOG_PATH}\n`));
  return { saved: true, path: CHANGELOG_PATH };
}
 ...
 *
 * | semantic | headless | pendingTag | function called                        |
 * |----------|----------|------------|----------------------------------------|
 * | false    | false    | any        | buildChangelog (manual interactive)    |
 * | false    | true     | any        | buildChangelogHeadless (no-op)         |
 * | true     | false    | undefined  | buildSemanticChangelogHeadless         |
 * | true     | false    | set        | buildSemanticChangelogInteractive      |
 * | true     | true     | any        | buildSemanticChangelogHeadless         |
 */
export async function runChangelog(config, opts = {}) {
  const { headless = false, pendingTag, semanticChangelog } = opts;
  const semantic =
  ...
  if (!pendingTag) {
    return buildSemanticChangelogHeadless(config);
  }
